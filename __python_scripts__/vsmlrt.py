...
def DPIR(
    ...
    if (
        isinstance(backend, Backend.TRT) and
        not backend.force_fp16 and
        parse_trt_version(int(core.trt.Version()["tensorrt_version"])) < (11, 0, 0)
    ):
    ...
    if implementation_version == 2:
        if isinstance(backend, Backend.TRT):
            ...
            if (4, 0) <= (model_major, model_minor):
                ...
                if parse_trt_version(int(core.trt.Version()["tensorrt_version"])) < (11, 0, 0):
                    backend.custom_args.extend([
                        "--precisionConstraints=obey",
                        "--layerPrecisions=" + (
...
def convert_model(
    ...
    if not any((fp16, bf16)):
        raise ValueError("at least one target precision must be specified")

    if bf16:
        raise NotImplementedError("bf16 not supported yet")

    if input_format != output_format:
        raise ValueError("input format must be equal to output format")
    else:
        fp16_io = output_format == 1
...
def trtexec(
    ...
    if fp16 and trt_version < (11, 0, 0):
        args.append("--fp16")
    ...
    if not tf32 and trt_version < (11, 0, 0):
        args.append("--noTF32")
